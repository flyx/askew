...
var isValidIdentifier = regexp.MustCompile(`^[\p{Letter}_][\p{Letter}0-9]*$`).MatchString

func (t *template) gatherObjects(n *html.Node, indexList []int) {
	meta := extractTdcAttribs(n)
	switch n.DataAtom {
	case atom.Input:
...
func (p *processor) processTemplate(n *html.Node) {
	meta := extractTdcAttribs(n)
	if meta.kind == noTmpl {
		panic("<template> must have tdc:kind!")
	}
	...
	if meta.kind == componentTmpl {
		if len(meta.name) == 0 {
			panic("component <template> misses tdc:name!")
		}
...
func (p *processor) process(file string) {
	...
	nodes, err := html.ParseFragment(bytes.NewReader(contents), nil)
	if err != nil {
