...
func (r *goRenderer) writeComponentFile(name string, c *component) {
	...
	for i := range c.captures {
		...
		for pName := range h.params {
			...
			fmt.Fprintf(&b, "this.Call(\"getAttribute\", \"%s\")", cap.paramMappings[pName])
		}
		b.WriteString(") {\narguments[0].Call(\"preventDefault\")\n}\nreturn nil\n})\n")
		b.WriteString("runtime.WalkPath(o.root, ")
		writePathLiteral(&b, cap.path)
		fmt.Fprintf(&b, ").Call(\"addEventListener\", \"%s\", wrapper)\n", cap.event)
		b.WriteString("}\n")
	}
...
func (p *processor) dump(htmlPath, packagePath string) {
	...
	for i := range p.components {
		html.Render(htmlFile, p.components[i].strippedHTML)
	}
	htmlFile.Close()
